...
# Which llvm projects are needed, and what is the signature (SHA256sum)
# of their corresponding source tarballs.
declare -A PROJECTS
PROJECTS["llvm-project"]=9a56d906a2c81f16f06efc493a646d497c53c2f4f28f0cb1f3c8da7f74350254
...
prepare_build() {
  ...
  for p in "${!PROJECTS[@]}"; do
    aws s3 cp --region=us-west-2 s3://sfc-eng-jenkins/xp-dependencies/llvm/$p-$CLANG_VERSION.src.tar.xz .
    # validate the signature of this tarball
    SIG_CMP=$(sha256sum $p-$CLANG_VERSION.src.tar.xz | cut -d ' ' -f1)
    if [ $SIG_CMP != ${PROJECTS[$p]} ]; then
      echo "Could not validate downloaded file $p-$(CLANG_VERSION).src.tar.xz".
      exit -1
    fi
  done
  cd ..
